...
@Component({
  selector: 'maat',
  templateUrl: './maat.component.html',
  styleUrls: ['./maat.component.css']
})
export class MaatComponent implements OnInit {
  ...
  maaKeys(maa:Object){
    return Object.keys(maa);
  }

  filteredMaaKeys(maa:Object){
    return this.maaKeys(maa).filter((value, index, arr) => {
      return !(value === "ID");
    });
  }

  filteredMaaValues(maa:Object){
    return Object.values(maa).filter((value, index, arr) => { 
      let key = this.maaKeys(maa)[index];
      
      if(key === "ID"){
        return false;
      }else if(key === "position" && value % 2 !== 0){
        return false;
      }
      return true;      
    });
